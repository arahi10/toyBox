...
def is_valid_chain(chain_table: 'list[list[int]]', cushion_indexes=set()):
    if ZABUTON:
        if chain_table[1][2] ==YELLOW or chain_table[0][3] == YELLOW:
            return False
...
if __name__ == '__main__':
    RESULT_FILE = "all.txt"
    URL_DUMP_FILE = "urls.txt"
    URL_DUMP_FILE_WITH_FAILURE = "urls.NGs.txt"
    ...
    translates = []
    if ZABUTON:
        base_dict = {1: 4, 2: 1, 3: 2}
        for i in product([3, 1, 2, 4], [4, 2, 1, 3], [1, 3, 2, 4]):
            add_dict = {j: i[j-4] for j in range(4, RENSA+1 + 1)}
            translates.append({**base_dict, **add_dict})
    else:
        base_dict = {1: 1, 2: 2}
        for i in product([3, 1, 2, 4], [4, 2, 1, 3], [1, 3, 2, 4]):
            add_dict = {j: i[j-3] for j in range(3, RENSA + 1)}
            translates.append({**base_dict, **add_dict})
    ...
    for index in tqdm(range(num)):
        ...
        head += 1+MAXDAN+1
    with open(URL_DUMP_FILE, 'w') as file:
        for k, v in sorted(URLs.items(), key=itemgetter(0)):
            file.write(f"{k}:{v}\n")
    with open(URL_DUMP_FILE_WITH_FAILURE, 'w')as file:
        NGs.sort()
        file.write('\n'.join(map(str, NGs)))
